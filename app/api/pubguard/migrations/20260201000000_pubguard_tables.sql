...
CREATE TABLE IF NOT EXISTS pubguard_scans (
  ...
  agent_id TEXT REFERENCES kira_agents(id) ON DELETE SET NULL,
...
-- Policy: Allow authenticated users to read their own scans (via agent ownership)
-- Adjust based on your auth setup
CREATE POLICY "Users can view scans from their agents" ON pubguard_scans
  FOR SELECT USING (true); -- Adjust this based on your auth model
