...
export default function ChatPage() {
  ...
  const userId = 'TODO_GET_FROM_AUTH'; // Replace with actual auth
    ...
    async function loadContext() {
      try {
        ...
        const res = await fetch(
          `/api/kira/conversation/context?agentId=${agentId}&userId=${userId}&limit=30`
        );
        ...
        const data: ConversationContext = await res.json();
        setContext(data);
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
      ...
      <header className="bg-white border-b px-4 py-3 flex items-center gap-3">
        ...
        />
        <div>
          <h1 className="font-semibold text-gray-900">Kira</h1>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 ? (
          <div className="text-center text-gray-500 mt-8">
            <p>Start your conversation with Kira!</p>
          </div>
        ) : (
